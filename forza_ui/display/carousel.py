...
class MGuidPrivate(QtWidgets.QFrame):
    ...
    def set_checked(self, value):
        bg_color = forza_theme.primary_color if value else forza_theme.background_color
...
@property_mixin
class FCarousel(QtWidgets.QGraphicsView):
    def __init__(self, pix_list, autoplay=True, width=500, height=500, parent=None):
        super(FCarousel, self).__init__(parent)
        self.scene = QtWidgets.QGraphicsScene()
        self.scene.setBackgroundBrush(QtGui.QBrush(QtGui.QColor(forza_theme.background_color)))
