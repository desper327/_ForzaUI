# -*- coding: utf-8 -*-
"""Decorator mixins for widget polish and animation."""
...
def cursor_mixin(cls):
    ...
    def _revert_cursor(self):
        if self.__dict__.get("__fui_enter", False):
            while self.__dict__.get("__fui_enter_count", 0) > 0:
                QtWidgets.QApplication.restoreOverrideCursor()
                self.__dict__.update({"__fui_enter_count": self.__dict__.get("__fui_enter_count", 0) - 1})
            self.__dict__.update({"__fui_enter": False})

    def _new_enter_event(self, *args, **kwargs):
        self.__dict__.update({"__fui_enter": True})
        self.__dict__.update({"__fui_enter_count": self.__dict__.get("__fui_enter_count", 0) + 1})
...
def focus_shadow_mixin(cls):
    ...
    def _new_focus_in_event(self, *args, **kwargs):
        ...
        if not self.graphicsEffect():
            ...
            fui_type = self.property("fui_type")
            color = vars(forza_theme).get("{}_color".format(fui_type or "primary"))
