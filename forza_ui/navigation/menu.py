...
from forza_ui.layout.popup import FPopup
from forza_ui.input.line_edit import FLineEdit
from Qt.QtWidgets import QProxyStyle
import forza_ui.utils as utils


def _qaction_group(parent):
    """Qt.py 在 PySide6 下可能未暴露 QActionGroup，回退到原生绑定。"""
    cls = getattr(QtWidgets, "QActionGroup", None) or getattr(QtGui, "QActionGroup", None)
    if cls is not None:
        return cls(parent)
    import importlib
    import Qt as qt_module

    mod = importlib.import_module(f"{qt_module.__binding__}.QtGui")
    return mod.QActionGroup(parent)


PYSIDE6 = os.environ.get("QT_API") == "pyside6"


class CompatStyle(QProxyStyle):
    """PySide2 PySide6"""
...
@property_mixin
class FScrollableMenuBase(QtWidgets.QMenu):
    """
    ...
    def __init__(self, *args, **kwargs):
        super(FScrollableMenuBase, self).__init__(*args, **kwargs)
    ...
    def actionRects(self):
        if self.dirty or not self._actionRects:
            ...
            for action in self.actions():
                geo = super(FScrollableMenuBase, self).actionGeometry(action)
                if offset:
    ...
    def isScrollable(self):
        ...
        size_hint = super(FScrollableMenuBase, self).sizeHint()
    ...
    def sizeHint(self):
        hint = super(FScrollableMenuBase, self).sizeHint()
    ...
    def eventFilter(self, source, event):
        if event.type() == QtCore.QEvent.Show:
            ...
            return False
        return super(FScrollableMenuBase, self).eventFilter(source, event)

    def event(self, event):
        if not self.isScrollable():
            return super(FScrollableMenuBase, self).event(event)
        if event.type() == event.KeyPress and event.key() in (
            QtCore.Qt.Key_Up,
            QtCore.Qt.Key_Down,
        ):
            res = super(FScrollableMenuBase, self).event(event)
    ...
    def timerEvent(self, event):
        if not self.isScrollable():
            ...
            super(FScrollableMenuBase, self).timerEvent(event)

    def mouseMoveEvent(self, event):
        if not self.isScrollable():
            super(FScrollableMenuBase, self).mouseMoveEvent(event)
            return
    ...
    def showEvent(self, event):
        if self.isScrollable():
            ...
            self.ignoreAutoScroll = False
        super(FScrollableMenuBase, self).showEvent(event)

    def hideEvent(self, event):
        for action in self.actions():
            if action.menu():
                action.menu().removeEventFilter(self)
        super(FScrollableMenuBase, self).hideEvent(event)

    def resizeEvent(self, event):
        super(FScrollableMenuBase, self).resizeEvent(event)
    ...
    def paintEvent(self, event):
        if not self.isScrollable():
            super(FScrollableMenuBase, self).paintEvent(event)
            return
...
@property_mixin
class FSearchableMenuBase(FScrollableMenuBase):
    def __init__(self, *args, **kwargs):
        super(FSearchableMenuBase, self).__init__(*args, **kwargs)
        self.search_popup = FPopup(self)
        self.search_popup.setVisible(False)
        self.search_bar = FLineEdit(parent=self)
    ...
    def search_key_event(self, call, event):
        ...
        if key in (
            ...
            super(FSearchableMenuBase, self).keyPressEvent(event)
    ...
    def keyPressEvent(self, event):
        ...
        return super(FSearchableMenuBase, self).keyPressEvent(event)


@property_mixin
class FMenu(FSearchableMenuBase):
    sig_value_changed = QtCore.Signal(object)

    def __init__(self, exclusive=True, cascader=False, title="", parent=None):
        super(FMenu, self).__init__(title=title, parent=parent)
        self.setProperty("cascader", cascader)
        self.setCursor(QtCore.Qt.PointingHandCursor)
        self._action_group = _qaction_group(self)
    ...
    def _add_menu(self, parent_menu, data_dict):
        if "children" in data_dict:
            menu = FMenu(title=data_dict.get("label"), parent=self)
